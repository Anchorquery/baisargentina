export '/backend/schema/util/schema_util.dart';

export 'booking_struct.dart';
export 'booking_verification_struct.dart';
export 'bookings_struct.dart';
export 'category_struct.dart';
export 'commerce_perfil_struct.dart';
export 'discount_struct.dart';
export 'discounts_struct.dart';
export 'error_struct.dart';
export 'estaditica_struct.dart';
export 'event_struct.dart';
export 'events_struct.dart';
export 'housing_struct.dart';
export 'organizador_struct.dart';
export 'owner_struct.dart';
export 'service_struct.dart';
export 'services_category_struct.dart';
export 'services_list_struct.dart';
export 'user_struct.dart';
export 'verificacion_data_struct.dart';
export 'file_d_struct.dart';
export 'pagination_struct.dart';
export 'plan_struct.dart';
export 'user_register_struct.dart';
